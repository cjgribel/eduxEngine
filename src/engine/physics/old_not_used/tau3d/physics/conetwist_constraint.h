...
#ifndef tau3d_conetwist_constraint_h
#define tau3d_conetwist_constraint_h

#include "constraint.h"



class conetwist_constraint_t : public angular_constraint_t
{
public:
    mat3f frameAw;                              // World frames
    mat3f frameBw;
    
    vec3f Axw, Azw, Byw, Bzw;                   // World frame axes
    vec3f Ayw, Bxw;
    
    float cone_max = 0;                         // Angle limits
    float twist_max = 0;
    
    vec3f v_cone, v_twist;                      // Constraint vectors
    ...
    conetwist_constraint_t(body_t *bodyA,
                           body_t *bodyB,
                           linear_constraint_t* pos_ctr = nullptr)
    : angular_constraint_t(bodyA, bodyB), pos_ctr(pos_ctr)
    {
    ...
    void set_limits(float cone_max,
                    float twist_max,
                    const mat3f &frameA = mat3f(1),
                    const mat3f &frameB = mat3f(1))
    {
...
//        printf("ax %f, ay %f\n", ax*fTO_DEG, ay*fTO_DEG);
        ...
        bias_cone = C_cone * conetwist_ctr_ERP * SIM_IDT;
...
#endif
        // Transform B to A
        mat3f frameBw_A = R * frameBw;
        // Now check x-x angle (twist)
        vec3f Bxw_A = frameBw_A.column(0);
...
//        printf("twist %f\n", twist_theta*fTO_DEG);
        ...
        bias_twist = C_twist * conetwist_ctr_ERP * SIM_IDT;
    ...
    void solve()
    {
        ...
        if (cone_ctr_active)
        {
            ...
            float Cdot_cone = (bodyB->W - bodyA->W).dot(v_cone);
        ...
        {
            // velocity constraint
            float Cdot_twist = (bodyB->W - bodyA->W).dot(v_twist);
