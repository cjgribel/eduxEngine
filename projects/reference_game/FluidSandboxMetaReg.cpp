...
namespace eeng::reference_game
{
    ...
    {
        EENG_LOG_INFO(&ctx, "Registering reference_game fluid sandbox meta types...");

        entt::meta_factory<eeng::reference_game::ecs::FluidFrameRenderMode>{}
            .custom<TypeMetaInfo>(TypeMetaInfo{
                .id = "eeng.reference_game.ecs.FluidFrameRenderMode",
                .name = "FluidFrameRenderMode",
                .tooltip = "Visualization mode for a 2D fluid frame." })
            .traits(MetaFlags::none);
        eeng::meta::register_type<eeng::reference_game::ecs::FluidFrameRenderMode>();
        warm_start_meta_type<eeng::reference_game::ecs::FluidFrameRenderMode>();

        entt::meta_factory<eeng::reference_game::ecs::FluidFrameComponent>{}
            .custom<TypeMetaInfo>(TypeMetaInfo{
                .id = "eeng.reference_game.ecs.FluidFrameComponent",
                .name = "FluidFrameComponent",
                .tooltip = "Project-local 2D incompressible fluid frame." })
            .traits(MetaFlags::none)

            .data<&eeng::reference_game::ecs::FluidFrameComponent::name>("name"_hs)
            .custom<DataMetaInfo>(DataMetaInfo{ "name", "Name", "Debug/display name." })
            .traits(MetaFlags::none)

            .data<&eeng::reference_game::ecs::FluidFrameComponent::enabled>("enabled"_hs)
            .custom<DataMetaInfo>(DataMetaInfo{ "enabled", "Enabled", "Enable simulation and rendering." })
            .traits(MetaFlags::none)

            .data<&eeng::reference_game::ecs::FluidFrameComponent::frame_size>("frame_size"_hs)
            .custom<DataMetaInfo>(DataMetaInfo{ "frame_size", "Frame Size", "Frame extents in the local XY plane." })
            .traits(MetaFlags::none)

            .data<&eeng::reference_game::ecs::FluidFrameComponent::resolution>("resolution"_hs)
            .custom<DataMetaInfo>(DataMetaInfo{ "resolution", "Resolution", "Grid resolution (nx, ny)." })
            .traits(MetaFlags::none)

            .data<&eeng::reference_game::ecs::FluidFrameComponent::simulation_rate>("simulation_rate"_hs)
            .custom<DataMetaInfo>(DataMetaInfo{ "simulation_rate", "Simulation Rate", "Scale factor applied to frame dt." })
            .traits(MetaFlags::none)

            .data<&eeng::reference_game::ecs::FluidFrameComponent::max_step_dt>("max_step_dt"_hs)
            .custom<DataMetaInfo>(DataMetaInfo{ "max_step_dt", "Max Step Dt", "Safety clamp for the internal solver dt." })
            .traits(MetaFlags::none)

            .data<&eeng::reference_game::ecs::FluidFrameComponent::substeps>("substeps"_hs)
            .custom<DataMetaInfo>(DataMetaInfo{ "substeps", "Substeps", "Number of internal solver substeps." })
            .traits(MetaFlags::none)

            .data<&eeng::reference_game::ecs::FluidFrameComponent::config_path>("config_path"_hs)
